...
and
\noindent\begin{align*}
    G(s) & = \mathbf{C}(s \mathbf{I}-\mathbf{A})^{-1}\mathbf{B} \text{ (reference to output)}        \\
    L(s) & = \mathbf{K}(s \mathbf{I}-\mathbf{A})^{-1}\mathbf{B} \text{ (reference to control input)}
\end{align*}
...
\begin{itemize}
    ...
    \item The gain margin is ($6\text{ dB}, +\infty \text{ dB}$)
\end{itemize}
