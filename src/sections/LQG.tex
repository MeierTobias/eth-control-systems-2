\section{\texorpdfstring{Linear-Quadratic Gaussian Compensator (LQG) / \\ Dynamic output Feedback Compensator (DOFC)}{Linear-Quadratic Gaussian Compensator (LQG) / Dynamic output Feedback Compensator (DOFC)}}
...
\end{itemize}
Remember however, that the systems should be LTI and that LQE assumes Gaussian white noise.
...
To ensure a DC-Gain of $1$ we need the reference scaling matrix $\mathbf{S}$ to satisfy
...
\begin{align*}
    \lim_{z\to1}\mathbf{G}_{yr}^{cl}(z)                              & = \mathbf{I} \\
...
The transfer function from $y$ to $u$ i.e.\ the TF of the compensator is
\begin{equation*}
    G_{uy} = -\begin{bmatrix}
    ...
    \end{bmatrix}
\end{equation*}
wich can be used for classical observations such as Bode plot.
% \textbf{Remark}
% $u_0=\mathbf{KS}r = \tilde{N}r$ can be considered a \textit{feedforward} control input.

\subsection{Margins}

While LQG/DFOC ensures the closed-loop stability of the nominal system, the stability margins can be \textbf{arbitrarily small}. Therefore, stability margins always need to be considered when working with LGQ.
