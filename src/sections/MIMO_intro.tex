...
\begin{enumerate}
    ...
    \item Find largest common numerator $n_{\max}(s)$ (usually product of $(s-z_i)$ terms) of the normalized minors from 4.
...
Assuming $\mathbf{A}$ has rank = smaller dimension:
\begin{enumerate}
    ...
    \item calculate eigenvectors of $\mathbf{U},\mathbf{V}$
          \noindent\begin{align*}
               & \mathbf{U}: & \left(\lambda_{\mathbf{U}} \mathbf{I} -\mathbf{AA}^{\mathsf{H}}\right)\mathbf{x}           & \overset{!}{=}\mathbf{x} \\
               & \mathbf{V}: & \left(\lambda_{\mathbf{V}} \mathbf{I} -\mathbf{A}^{\mathsf{H}} \mathbf{A}\right)\mathbf{x} & \overset{!}{=}\mathbf{x}
          \end{align*}
...
\end{equation*}
shows that a input in direction $\mathbf{u}_i$ results in an output in direction $\mathbf{v}_i$ with amplification $\sigma_i$.
...
\paragraph[H_2-induced Norm]{$\mathcal{H}_2$-norm}
...
\textbf{Remarks}:
\begin{itemize}
    \item The LGR/LQE problem can be though as a $\mathcal{H}_2$ minimization problem.
