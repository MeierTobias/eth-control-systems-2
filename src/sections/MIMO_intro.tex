...
\begin{examplesection}[Example (From Script of Gioele Zardini)]
    ...
    % TODO: How do we find the corresponding direction? -> see extension above. We could add it for this example as well
\end{examplesection}
...
\begin{itemize}
    ...
    \item Given zero initial condition and excitation $\mathbf{u}(t)=\mathbf{u}_i e^{s_i t}$ (zero direction and zero frequency) we get
          ...
          \end{equation*} which contains no components of the input and becomes $\mathbf{0}$ for a corresponding non-zero initial condition.    % TODO: Doesn't the latter depend on the eigenvalues of A (poles)?
...
\begin{enumerate}
    ...
    \item Calculate the ranks $r_i = \text{rank}(\mathbf{R_i})$ which indicate the number of poles at location $p_i$ that are needed.
          \begin{itemize}
              ...
              \item Check $n\le n_d$ after calculating $r_i$. % TODO: This is the opposite of the bullet point above (one of them is wrong)
          \end{itemize}
...
% TODO: Should we add the proofs why p-norms are norms?
...
\begin{itemize}
    \item In words, the maximum amplification of the \textit{signal} energy in time domain corresponds to the maximum singular value of the \textit{system} it propagates through.   % TODO: Correct?
    \item In SISO systems, the $\mathcal{H}_\infty$ norm corresponds to the peak value of the Bode plot.    % TODO: i.e.\ energy amplification in time domain correlates with magnitude peak in frequency domain? Kind of wild
