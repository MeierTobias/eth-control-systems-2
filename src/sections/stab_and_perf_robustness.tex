...
\end{itemize}
the FB interconnection of $\mathbf{G}$ and $\mathbf{\Delta}$ is \textbf{stable iff}
\begin{equation*}
...
\end{equation*}
However, this is a conservative assumption as the SGT could be applied to an arbitrary $\mathbf{\Delta}$. Therefore, we adjust condition to more specific uncertainty matrices.
...
\begin{equation*}
    \mu(\mathbf{M}):=\frac1{\inf\{\sigma_{\max}(\Delta):\det(1-M\Delta)=0\}},\quad\Delta\in\mathbb{D}
\end{equation*}
\begin{itemize}
    \item By convention, if $\det(I-M\Delta)\neq0,\forall\Delta\in\mathbb{D}\text{, then }\mu(M)=0$.
    \item If $\Delta$ is diagonal and $M$ has rank one the SSV can be computed analytically.
\end{itemize}
