...
\begin{equation*}
    \lim_{k\to+\infty}y[k]=G(1)u_0=(\mathbf{C}{(1-\mathbf{A})}^{-1}\mathbf{B}+\mathbf{D})u_0
\end{equation*}
...
\begin{align*}
    \mathbf{A}_d & :=e^{\mathbf{A}T} & \mathbf{B}_d & :=\left(\int_0^T {e^{\mathbf{A}\theta}}d\theta\right)\mathbf{B} \overset{\mathbf{A} \text{ inv.}}{=} \mathbf{A}^{-1}\left(\mathbf{A}_d-\mathbf{I}\right)\mathbf{B} \\
