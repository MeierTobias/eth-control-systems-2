...
\end{align*}
Alternatively the DT state space matrices $\mathbf{A}_d, \mathbf{B}_d, \mathbf{C}_d, \mathbf{D}_d$ can be used to compute the DT transfer function $G_{d}(z)$ and the corresponding difference equation can be derived using the z transform. %TODO: This belongs to the section "Transfer Functions (TF) of DT LTI Systems" I would suggest we add the subscript _d to the formulas there (which probably caused the confusion) and remove this comment.
