\subsection{\texorpdfstring{$\mathcal{H}_2$}{H2} Synthesis}\label{h2_synth}
...
\begin{itemize}
    ...
    \item $\mathbf{C}_z^{\mathsf{T}}\mathbf{D}_{zu}=0$  % TODO: Intuition. Means what?
\end{itemize}
...
\begin{enumerate}
    \item $(\mathbf{A},\mathbf{B}_u)$ stabilizable.     % TODO: If this is only stabilizable but not controllable: Does it mean that LQR yields something that is optimal for the given functional but can still yield a ridiculously slow CL system because of uncontrollable modes? (Probably yes and this would still be as good as it gets :))
