...
\begin{itemize}
    \item $\mathbf{Q}$
          ...
          \end{itemize}
    \item $\mathbf{R}$ must be positive definite i.e. $\mathbf{u}^{\mathsf{T}} \mathbf{Ru} >0\; \forall \mathbf{u}\neq \mathbf{0}$ or $eig(\mathbf{R}) > 0$ (otherwise not every control effort is penalized).
          % TODO: Why must Q but not R be symmetric? -> I would say R must also be symmetric otherwise the the dimensions of the multiplication would not match.
    \item The pair $\mathbf{A,B}$ must be stabilizable
          % TODO: Why not reachable? -> because the unreachable modes will vanish.
...
\begin{itemize}
    \item LQR guarantees a phase margin of at least 60° and gain margin in $\left[\frac{1}{2},\infty\right)$ % ChkTeX 9 % TODO: I would remove this here because it is already stated in the margin section.
          \begin{itemize}
              \item However, robustness is limited (see\ \cite{539438}) % TODO: How is that? I have not read the paper you cited but isn't robustness defined as a guaranteed margin to instability? And also the same as below in the margin section (do we need the citation).
          \end{itemize}
...
Feed-forward reference scaling is not \textbf{robust}, e.g.\ to changes/uncertainties in the plant dynamics or disturbances. If the reference input is $\neq 0$, an integrator can be used to achieve zero-steady error to step inputs. % TODO: I would say the reference input can also be zero and there could still be a need to ad an integrator to get rid of the tracking error.
...
\begin{itemize}
    ...
    \item Note however the limitations e.g.\ \cite{539438}  % TODO: What limitations? If they are not important enough to be stated I would say we don't need them in de summary. Do we really need the citation here? All the other statements are also just given from the lecture without further sources. I would suggest if in the end this is the only citation we ist omit it.
\end{itemize}
