...
\textbf{Remarks:}
\begin{itemize}
    \item $\mathbf{Q}$ is symmetric i.e. $\mathbf{Q}=\mathbf{Q}^{\mathsf{T}}$
    \item $\mathbf{Q}$ is positive semidefinite i.e. $\mathbf{x}^{\mathsf{T}} \mathbf{Qx} \geq 0$ or $eig(\mathbf{Q}) \geq 0$
    ...
    \item $\mathbf{R}$ must be positive definite i.e. $\mathbf{u}^{\mathsf{T}} \mathbf{Ru} >0\; \forall u\neq 0$ or $eig(\mathbf{R}) > 0$
