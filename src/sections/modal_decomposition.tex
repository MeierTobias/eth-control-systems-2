...
which means that
\begin{itemize}
    \item each mode evolves independently (of other modes) over time.
    \item $\tilde{\mathbf{x}}(t)$ is obtained elementwise from $\tilde{x}_i(t)$.
\end{itemize}

\newpar{}
\ptitle{In Standard Coordinates}

The modes in standard coordinates are given by
\begin{equation*}
    \mathbf{m}_i(t) = e^{\lambda_{i}t}\tilde{x}_i(0)\mathbf{v}_i
\end{equation*}
so that
\begin{equation*}
    \mathbf{x}(t)=\sum_{i=1}^{n} \mathbf{m}_i(t) = \sum_{i=1}^{n}e^{\lambda_{i}t}\tilde{x}_i(0)\mathbf{v}_i
\end{equation*}
