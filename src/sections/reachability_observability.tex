...
% TODO: I would suggest to move this part into the corresponding subsections below because most of the information is already there and not jet introduced at this point.
...
\begin{align*}
    \text{rank}(\mathbf{R}) & = n                            \\
    \det(\mathbf{R})        & \neq 0                         \\  % TODO: Maybe remove this and the line below. (I think only the Rank operator is correct.)
...
\ptitle{Remarks}
\begin{itemize}
    \item In the latter case, the (possibly underdetermined) linear system can be solved for $\mathbf{U}$ which yields the desired input sequence to fullfil the condition. % TODO: What does this mean? 
...
\begin{itemize}
    \item A DT LTI system is controllable if, for any initial condition $\mathbf{x}_0$, there exists a control input that brings the state $x$ to $0$ in finite time (Note: For CT one has the ``opposite'' condition). % TODO: This is wrong: Reachability is the ``opposite'' condition.
    ...
    \item An unreachable DT system with non-invertible $\mathbf{A}_d$ could be controllable (Eigenvalues at 0). E.g.:
          ...
          \end{itemize}
          % TODO: Can an uncontrollable system be reachable? (I don't think so) -> NO 
\end{itemize}
...
% TODO: Is it right that these two conditions are just a "weaker" subset of Reachability/Observability? -> NO, they are super sets (a reachable system is always stabilizable but an unreachable system can be stabilizable if the unreachable modes are stable.)
