...
\textit{Lyapunov functions are, in a sense, a notion of energy: non-negative, minimized (0) only at the equilibrium point and non-increasing along all trajectories.} % TODO: non-increasing in t but could be increasing in x ???
\newpar{}
...
\begin{itemize}
    ...
    \item $V(\mathbf{x})=0$ must hold in any equilibrium point, even if there are more then one.
          ...
          %TODO:: x eventually approaches the largest pos. inv. set. in which Vdot=0. But are there also pos. inv. sets in which Vdot \neq 0?
