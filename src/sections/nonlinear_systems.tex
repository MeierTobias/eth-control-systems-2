...
\end{itemize}
General Lyapounov functions are
\begin{itemize}
    \item Energy
...
\begin{itemize}
    ...
    \item $V(\mathbf{x})=0$ must hold in any equilibrium point, even if there are more then one.
          %TODO:: must the "only equilibrium point" from above be (0,...0)?
          %TODO:: x eventually aproaches the largest pos. inv. set. in which Vdot=0. But are there also pos. inv. sets in which Vdot \neq 0?
...
i.e.\ using the control iput $u$, $V$ becomes a Lyapounov function. In the following methods, one aims to find a control Lyapunov function to prove stabilizing behavior with a specific control input.
