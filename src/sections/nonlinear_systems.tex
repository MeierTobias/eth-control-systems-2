...
\noindent\begin{equation*}
    \dot{V}(\mathbf{x}(t)) = 0 \text{ only if } \mathbf{x}(t) = \mathbf{x}_e
\end{equation*}
...
\begin{itemize}
    ...
    \item $V(\mathbf{x})=0$ must hold in any equilibrium point, even if there are more then one.
          %TODO:: must the "only equilibrium point" from above be (0,...0)? -> I think not -> if multiple equilibria are possible, they can't all be 0 -> doesn't have to be 0
...
Using $\dot{\mathbf{u}}_0=\dot{\mathbf{z}}-\dot{\mathbf{e}}$ one finally finds the stabilizing control law as
\begin{align*}
