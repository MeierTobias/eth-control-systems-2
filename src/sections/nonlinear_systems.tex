...
\newpar{}
Let $\Omega\in D$ be a (compact) positively invariant set ($\mathbf{x}$ stays in the set) i.e., $\mathbf{x}(t_0)\in \Omega \Rightarrow \mathbf{x}(t)\in\Omega\; \forall t \geq t_0$.
...
In the following methods, one aims to find a control Lypunov function to prove stabilizing behavior with a specific control input.
...
\begin{align*}
    ...
    L_g L_f h(x) & := L_g (L_f h(x)) = \frac{\partial L_f h(x)}{\partial x}g(x)
\end{align*}
...
\begin{gather*}
    ...
    y = h(x,u)
\end{gather*}
is differentially flat, with flat output $y$, if one can compute the state and input trajectories as a function (map $\Xi$) of the flat outputs and a finite number of derivatives.
\begin{equation*}
    (x,u) = \Xi (y,\dot{y}, \ldots, y^{(m)})
\end{equation*}
